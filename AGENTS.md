...
2. **No Silent Changes to User Notes**: Schema migrations or index rebuilds must never silently overwrite or modify on-disk `.md` files without an explicit edit/delete command from the user.
3. **No Implicit Default Project**: There is no fallback project. If a command cannot resolve project context from flags, environment variables (`MNEMONIC_PROJECT`), or local `.mnemonic` discovery, it must fail with an error.
4. **XDG Directory Separation**:
   - Configuration -> `XDG_CONFIG_HOME/mnemonic/config.toml`
   - Global Registry -> `XDG_DATA_HOME/mnemonic/registry.sqlite`
...
- `CodeAmbiguous` = 4 (Multiple projects resolved, duplicate slugs)
