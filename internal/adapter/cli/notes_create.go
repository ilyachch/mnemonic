...
func newNotesCreateCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create a note",
		RunE:  runNotesCreate,
	}
	cmd.Flags().String("title", "", "note title")
	cmd.Flags().Bool("stdin", false, "read the note body from stdin")
	cmd.Flags().String("body-file", "", "read the note body from a file")
	cmd.Flags().StringArray("tag", nil, "add a note tag")
	return cmd
}
...
func runNotesCreate(cmd *cobra.Command, args []string) error {
	...
	return PrintOutput(cmd.OutOrStdout(), jsonOutputEnabled(cmd), created.Path+" created\n", newNotesCreateOutput(created))
}
